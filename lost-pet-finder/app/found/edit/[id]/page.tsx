...
export default function EditFoundPet() {
  ...
  const [currentPetLocationType, setCurrentPetLocationType] = useState<string>("")
  ...
  useEffect(() => {
    const fetchPet = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/FoundPets/${petId}`);
        if (!response.ok) {
        ...
        setDateFound(new Date(petData.foundDateTime).toISOString().split('T')[0]);
        setExistingImages(petData.images || []);
        setCurrentPetLocationType(petData.currentPetLocationType || "with-finder");
        
        // Set location
        if (petData.foundLocation) {
          setLocation({
            address: petData.locationName || "",
            latitude: petData.foundLocation.latitude,
            longitude: petData.foundLocation.longitude,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!location.latitude || !location.longitude) {
      toast({
        ...
        description: "Please select the location where the pet was found",
        variant: "destructive"
      })
      return
    }
    ...
    if (!dateFound) {
      toast({
        title: "Missing Date",
        description: "Please enter the date when the pet was found",
        variant: "destructive"
      })
      return
    }
    ...
    if (!currentPetLocationType) {
      toast({
        ...
        description: "Please select the current location of the pet",
        variant: "destructive"
      })
      return
    }
    ...
    if (!token) {
      toast({
        ...
        description: "Please log in to update a found pet report",
        variant: "destructive"
      })
      router.push('/login?redirect=found/edit/' + petId)
      return
    }
    ...
    try {
      ...
      toast({
        ...
        description: "Your found pet report has been updated successfully",
        variant: "default"
      })
      ...
      console.error('Error updating found pet report:', error)
      toast({
        title: "Update Failed",
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col min-h-screen">
        <SiteHeader />
        <main className="flex-1 bg-pet-warm/30">
          <div className="container px-4 py-12 md:px-6 md:py-16 lg:py-24">
            <div className="mb-8">
              <Link
                href="/profile"
                className="inline-flex items-center gap-2 text-sm font-medium text-pet-accent hover:text-pet-accent/80"
              >
            ...
            <div className="flex justify-center items-center py-20">
              <div className="text-center">
                <Loader2 className="h-12 w-12 animate-spin mx-auto mb-4 text-pet-accent" />
                <h2 className="text-2xl font-semibold text-pet-accent mb-2">Loading pet information...</h2>
                <p className="text-muted-foreground">Please wait while we fetch the pet's data.</p>
              </div>
            </div>
          </div>
        </main>
          ...
          <div className="container px-4 py-12 md:px-6 md:py-16 lg:py-24">
            ...
            <div className="flex justify-center items-center py-20">
              <div className="text-center">
                ...
                <Button onClick={() => router.push('/profile')} className="bg-pet-accent hover:bg-pet-accent/90">
                  Return to Profile
                </Button>
              </div>
            </div>
            ...
            <Card className="border-pet-primary/10 shadow-lg pet-card-hover">
              <CardHeader className="bg-gradient-to-r from-pet-warm to-white border-b border-pet-primary/10">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <CardTitle className="text-2xl text-pet-accent">Edit Found Pet Report</CardTitle>
                </div>
                <CardDescription className="text-muted-foreground">
                  Update the information about the pet you found to help reunite it with its owner.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-6 pt-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="found-location" className="text-pet-primary font-medium">
                      Found Location
                    </Label>
                        ...
                        >
                          {locationSuggestions.map((suggestion) => (
                            <div
                              ...
                              className="cursor-pointer px-4 py-2 hover:bg-pet-warm/20"
                    ...
                    <div className="mt-2 space-y-2">
                      <p className="text-sm text-muted-foreground">
                        Search for an address above or click on the map to pinpoint the exact location where you found the pet.
                      </p>
                      <div
                        ref={mapRef}
                        className="h-[300px] w-full pet-map-container"
                        style={{ background: mapLoaded ? "none" : "#fff5f0" }}
                      >
                        {!mapLoaded && (
                      ...
                      {location.latitude && location.longitude && (
                        <p className="text-xs text-pet-accent">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-2">
                      ...
                      >
                        <div className="text-center p-4">
                          <div className="mx-auto h-10 w-10 rounded-full bg-pet-warm/50 flex items-center justify-center">
                            <svg
                              ...
                              strokeWidth={1.5}
                              stroke="currentColor"
                              className="w-5 h-5 text-pet-accent"
                            >
                              <path
                                strokeLinecap="round"
                          ...
                          <p className="mt-2 text-xs font-medium text-pet-accent">Upload Photo</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex justify-end gap-4 pt-4 border-t border-pet-primary/10">
                  ...
                  <Button
                    type="submit"
                    className="bg-pet-accent hover:bg-pet-accent/90"
                    disabled={isSubmitting}
                  >
                    {isSubmitting ? (
                      <>
